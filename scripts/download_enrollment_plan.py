#!/usr/bin/env python3
"""
Download Monthly MA Enrollment by Plan from CMS
...
def get_url(year: int, month: int) -> str:
    ...
    if year >= 2020:
        return f"https://www.cms.gov/files/zip/monthly-plan-enrollment-{month_name}-{year}.zip"
    # 2018-2019: downloads.cms.gov
    elif year in [2018, 2019]:
        return f"https://downloads.cms.gov/files/ma-plan-enrollment-{year}-{month_num}.zip"
    # Older years
    else:
        return f"https://www.cms.gov/Research-Statistics-Data-and-Systems/Statistics-Trends-and-Reports/MCRAdvPartDEnrolData/Downloads/MA-Plan-Enrollment-{year}-{month_num}.zip"
...
def download_and_upload(year: int, month: int, dry_run: bool = False) -> dict:
    ...
    month_num = f"{month:02d}"
    s3_key = f"{S3_PREFIX}/{year}-{month_num}/enrollment_plan_{year}_{month_num}.zip"
    ...
    local_file = f"{LOCAL_TEMP}/enrollment_plan_{year}_{month_num}.zip"
...
def main():
    ...
    print("=" * 60)
    print("MA Enrollment by Plan Download")
    print("=" * 60)
    ...
    log_file = LOG_DIR / f"enrollment_plan_download_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
