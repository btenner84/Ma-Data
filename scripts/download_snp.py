#!/usr/bin/env python3
"""
Download SNP (Special Needs Plan) Comprehensive Reports from CMS
Total files: ~219 (Dec 2007 - Feb 2026)
"""
...
def get_snp_url(year: int, month: int) -> str:
    """Generate SNP URL based on year."""
    month_num = f"{month:02d}"
    month_name = MONTH_NAMES[month]

    # 2020+: New pattern
    if year >= 2020:
        return f"https://www.cms.gov/files/zip/snp-comprehensive-report-{month_name}-{year}.zip"
    # Older: Try downloads.cms.gov
    else:
        return f"https://downloads.cms.gov/files/snp-comprehensive-report-{year}-{month_num}.zip"
...
def download_and_upload(year: int, month: int, dry_run: bool = False) -> dict:
    url = get_snp_url(year, month)
    month_num = f"{month:02d}"
    s3_key = f"{S3_PREFIX}/{year}-{month_num}/snp_{year}_{month_num}.zip"
    ...
    local_file = f"{LOCAL_TEMP}/snp_{year}_{month_num}.zip"
...
def main():
    ...
    print("=" * 60)
    print("SNP Comprehensive Report Download")
    print("=" * 60)
    ...
    log_file = LOG_DIR / f"snp_download_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
